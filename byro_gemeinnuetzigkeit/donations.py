...
def generate_donation_receipt(member, year):
    fees = member.fee_payments.filter(transaction__value_datetime__year=year).aggregate(
        fees=models.Sum("amount")
    )["fees"] or Decimal("0.00")
    donations = member.donations.filter(
        transaction__value_datetime__year=year
    ).aggregate(donations=models.Sum("amount"))["donations"] or Decimal("0.00")
    ...
    our_title = Paragraph(
        "Aussteller (Steuerbegünstigte Einrichtung)", style["Heading5"]
    )
    story.append(
        Table(
            [
    ...
    story.append(
        Paragraph(
            "Bestätigung über Geldzuwendungen/Mitgliedsbeitrag", style["Heading3"]
        )
    )
    story.append(
        Paragraph(
    ...
    data = [
        [_("Product"), _("Tax rate"), _("Net"), _("Gross")],
    ]
    data = [["Art", "in Ziffern", "in Buchstaben", "Datum"]]
    if fees:
        data.append(
            ["Beiträge", CURRENCY.format(fees), lib_num2words(fees, lang="de"), year]
        )
    if donations:
        data.append(
            [
                ...
                lib_num2words(donations, lang="de"),
                year,
            ]
        )
    last_row = len(data) - 1
    ...
    story.append(
        Table(
            [
                [
                    Paragraph(
                        "Es wird bestätigt, dass die Zuwendung nur zur Förderung {zwecke} verwendet wird.".format(
                            zwecke=local_settings.reason
                        ),
                        style["Normal"],
                    )
    ...
    data = [
        [
            "{location}, {date}".format(
                location=local_settings.location,
                date=formats.date_format(now().date(), use_l10n=True),
            )
        ],
    ...
    doc.document.save(
        "spenden/spenden_{}_{}_{}_{}.pdf".format(
            year, member.number, member.name, random_string
        ),
